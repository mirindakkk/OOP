...
namespace Lab11
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nРавносторонних " + col + " Макс " + max + " Мин " + min);
            col = 0;
            min = 1111111111;
            max = -1111111111;
            ...
            Console.WriteLine("\nПрямоугольных " + col + " Макс " + max + " Мин " + min);
            col = 0;
            ...
            foreach (Triangle tr in dist)
            {
                Console.WriteLine(tr.ToString());
            }
            Console.WriteLine("\nunion");
            var uni = list.Union(diap);
            foreach (Triangle tr in uni)
            {
                Console.WriteLine(tr.ToString());
            }
            Console.WriteLine("\nexcept");
            var exc = list.Except(sorttr);
            foreach (Triangle tr in exc)
            {
                Console.WriteLine(tr.ToString());
            }
            Console.WriteLine("\nintersect");
            var inter = list.Intersect(sorttr);
            foreach (Triangle tr in inter)
            {
                Console.WriteLine(tr.ToString());
            }
        }
